import { updateErrorElementStyle, validateUsername, validatePassword, validateName, validateBirth, validatePhone, formatPhoneString, checkDuplEmail, checkDuplPhone, checkDuplUsername } from "./validation.js";
...
window.onload = () => {
	...
	phone.addEventListener("change", async (e) => {
		...
		isValidPhone = await checkDuplPhone(input);
	...
	email.addEventListener("change", async (e) => {
		...
		isValidEmail = await checkDuplEmail(input);
